(function () {
    ...
    /** @ngInject */
    function ArticleViewCtrl($scope, $state, $stateParams, $anchorScroll, $location, $http, $q, $filter, $uibModal, $rootScope, $window, $document, screenSize, toastr, themeLayoutSettings, dataService, URL, authenticationService) {
        ...
        var loadComments = function (pageNum, pageSize, query) {
            $http.post(URL.assemble(URL.ARTICLES_COMMENTS_QUERY, $stateParams.id, pageNum, pageSize), query).then(function (response) {
        ...
        if ($stateParams.id) {
            $http.get(URL.assemble(URL.ARTICLES_ID, $stateParams.id)).then(function (response) {
        ...
        $scope.thumbsUp = function () {
            $http.post(URL.assemble(URL.ARTICLES_THUMBS_UP, $stateParams.id)).then(function (response) {
        ...
        $scope.submitComment = function (form, isReplyToUser, repliedUserId, repliedUserType) {
            ...
            return $q(function (resolve, reject) {
                ...
                $http.post(URL.get(URL.ARTICLES_COMMENTS), comment).then(function (response) {
        ...
        $scope.hasUserOwnTravelogue = function () {
            // return $scope.travelogue.ownerId === authenticationService.getClaims().userId;
        ...
        $scope.deleteTravelogue = function () {
            ...
            var modalInstance = $uibModal.open({
                ...
                resolve: {
                    message: function () {
                        return '确认删除文章吗？操作不可恢复哦！';
                    }
                }
            });

            modalInstance.result.then(function () {
                $http.delete(URL.assemble(URL.ARTICLES_ID, $stateParams.id)).then(function (response) {
